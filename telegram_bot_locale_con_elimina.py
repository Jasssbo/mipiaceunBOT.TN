    ...
    try:
        ...
        if data.startswith("new_"):
            ...
            try:
                ...
                confirm_msg_id = user_data.get(user.id, {}).get("confirm_msg_id")
                if preview_id:
                    await safe_delete(client, user.id, preview_id)
                if confirm_msg_id:
                    await safe_delete(client, user.id, confirm_msg_id)
                await callback_query.edit_message_text("✅ Annuncio eliminato con successo.")
                await callback_query.answer("Hai cancellato il tuo annuncio.", show_alert=False)
                if user.id in user_data:
                    user_data[user.id].pop("confirm_msg_id", None)
                    user_data[user.id].pop("preview_msg_id", None)
